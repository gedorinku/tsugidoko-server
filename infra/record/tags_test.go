...
func testTagToManyClassRoomTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadClassRoomTags(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyUserTags(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TagSlice{&a}
	if err = a.L.LoadUserTags(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpClassRoomTags(t *testing.T) {
	var err error
...
func testTagToManyAddOpUserTags(t *testing.T) {
	var err error
...
var (
	tagDBTypes = map[string]string{`CreatedAt`: `timestamp without time zone`, `ID`: `bigint`, `Name`: `character varying`, `UpdatedAt`: `timestamp without time zone`}
	_          = bytes.MinRead
)
