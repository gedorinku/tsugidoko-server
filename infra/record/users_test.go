...
func testUserOneToOneUserPositionUsingUserPosition(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var foreign UserPosition
	var local User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, userPositionDBTypes, true, userPositionColumnsWithDefault...); err != nil {
	...
	check, err := local.UserPosition().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadUserPosition(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserPosition == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.UserPosition = nil
	if err = local.L.LoadUserPosition(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserPosition == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpUserPositionUsingUserPosition(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, userPositionDBTypes, false, strmangle.SetComplement(userPositionPrimaryKeyColumns, userPositionColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userPositionDBTypes, false, strmangle.SetComplement(userPositionPrimaryKeyColumns, userPositionColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*UserPosition{&b, &c} {
		err = a.SetUserPosition(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.UserPosition != x {
...
func testUserToManyUserTags(t *testing.T) {
	...
	userTag, err := a.UserTags().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range userTag {
		if v.UserID == b.UserID {
			bFound = true
		}
		if v.UserID == c.UserID {
...
var (
	userDBTypes = map[string]string{`CreatedAt`: `timestamp without time zone`, `ID`: `bigint`, `Name`: `character varying`, `PasswordDigest`: `character varying`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
